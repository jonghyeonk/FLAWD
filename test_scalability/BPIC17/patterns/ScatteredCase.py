...
def ScatteredCase(data: pd.DataFrame, log_name:str, syslist: str, tstart: datetime= None, tend: datetime= None, 
    ...
    if (tstart == None) & (tend == None):
        log_split1 = data[~(data[attr].isin(condition))].reset_index(drop=True)
        log_split2 = data[data[attr].isin(condition)].reset_index(drop=True)
        ...
        if sum( ((tstart > data[timestamp_key]) | (data[timestamp_key] > tend ))) ==0:
            raise ValueError( print("Error: no matched events in the time interval"))
