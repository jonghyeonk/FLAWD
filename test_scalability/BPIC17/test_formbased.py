...
rates = [i*3/100 for i in range(1,11)]
rates=[0.3]
list_time = []
list_memory = []

for rate in [ 0.2, 0.25, 0.3]: #0.05, 0.1, 0.15, 0.2, 0.25, 0.3  

    start_time = time.time()
    tracemalloc.start()

    EL_polluted = FormBased(EL,
                        which = ['O_Create Offer', 'O_Created', 'O_Sent (mail and online)'], 
                        ratio= float(rate) , 
                        case_id_key = "Case",
    ...
    with open('summary_Formbased.csv', 'a', encoding='utf-8', newline='') as csvfile:
