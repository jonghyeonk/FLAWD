...
def ScatteredEvent(data: pd.DataFrame, target: list, action:str, loc:str = None, Del:bool = False, ratio:float= None, 
    ...
    if (tstart == None) & (tend == None):
        ...
        if data.empty:
            ...
            print("Filtering step", step, ". The number of cases in the time interval (", tstart, ",", tend ,"): ", len(data[case_id_key].unique()))
        step += 1
            ...
            for part in form_parts:
                if part in form_attr:
                    ...
                    if 'a-zA-Z' in part:
                        ...
                        print("Error: wrong or unsupportable regular expression:" + part)
